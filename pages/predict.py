...
def air_quality(url):
    ...
    # st.write(f"Carbon Monoxide: {air_q['co']}\nno2: {air_q['no2']}\no3: {air_q['o3']}\nso2: {air_q['so2']}")
...
def current(key,city,base_url):
    ...
    if response.status_code == 200:
        ...
        if a1:
                ...
                st.write(f"Current temperature in {city}: {temp_c}°C")
                st.write(f"Current temperature in {city}: {temp_f}°F")
                # break
        if a2:
                phase = data["current"]["is_day"]
                if phase == 0:
                    st.write("It is night.")
                else:
                    st.write("It is day.")
                # break
        if a3:
...
if __name__ in "__main__":
    ...
    if 'current' in weather:
        current(key,city,base_url)
    if 'astronomy' in weather:
        astronomy(key,city,base_url)
    if 'forecast' in weather:
        forecast(key,city,base_url)
